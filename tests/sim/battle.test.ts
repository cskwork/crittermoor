...
import type { BattleCritter } from '@/game/Sim/Battle/BattleState'
import { createBattleState, isBattleOver } from '@/game/Sim/Battle/BattleState'
import { executeTurn } from '@/game/Sim/Battle/BattleSim'

function mk(level: number, atk: number, def: number, spd: number, types: [CritterType, CritterType | null], moves: string[] = ['bite']): BattleCritter {
  return {
    speciesId: 1,
    level,
    types,
...
describe('type chart', () => {
  it('is exhaustive (covers all type pairs)', () => {
    for (let a = 0; a < 8; a++) {
      for (let b = 0; b < 8; b++) {
        const m = typeMultiplier(a as CritterType, b as CritterType)
...
describe('damage formula', () => {
  it('scales monotonically with attacker attack', () => {
    fc.assert(
      fc.property(fc.integer({ min: 30, max: 100 }), fc.integer({ min: 30, max: 100 }), (atkLow, delta) => {
  ...
  it('scales inversely with defender defense', () => {
    fc.assert(
      fc.property(fc.integer({ min: 30, max: 100 }), fc.integer({ min: 30, max: 100 }), (defLow, delta) => {
...
describe('battle sim', () => {
  ...
  it('is deterministic for the same seed and inputs', () => {
    const make = () => createBattleState(
      [mk(15, 60, 50, 70, [CritterType.Fire, null], ['ember'])],
      [mk(15, 60, 50, 50, [CritterType.Plant, null], ['leaf_cut'])],
      createRng(31415),
    )
    let a = make()
    let b = make()
    for (let i = 0; i < 5; i++) {
      const actions: [import('@/game/Sim/Battle/BattleState').BattleAction, import('@/game/Sim/Battle/BattleState').BattleAction] = [
