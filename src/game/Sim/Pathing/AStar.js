...
export function aStar(req) {
    ...
    if (fromIdx === toIdx)
        return { ok: true, nodes: new Int16Array([fromX, fromY]), expanded: 0 };
    ...
    while (open.size > 0) {
        ...
        for (let dir = 0; dir < (allowDiag ? 8 : 4); dir++) {
            ...
            if (tileCost === 0 || closed[ni])
                continue;
            // prevent corner-cut through impassable
            if (dx !== 0 && dy !== 0) {
                if (cost[idx(cx + dx, cy, width)] === 0 || cost[idx(cx, cy + dy, width)] === 0)
                    continue;
            }
...
function heuristic(x, y, tx, ty, allowDiag) {
...
function reconstruct(cameFrom, end, width) {
    const path = [];
    let cur = end;
    while (cur !== -1) {
...
class MinHeap {
    ...
    sinkDown(i) {
        for (;;) {
            ...
            const r = 2 * i + 2;
            let smallest = i;
            if (l < this.size && this.scores[l] < this.scores[smallest])
                smallest = l;
            if (r < this.size && this.scores[r] < this.scores[smallest])
                smallest = r;
            if (smallest === i)
                break;
