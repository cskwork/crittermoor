...
export var JobKind;
(function (JobKind) {
    JobKind[JobKind["None"] = 0] = "None";
    JobKind[JobKind["Chop"] = 1] = "Chop";
    JobKind[JobKind["Mine"] = 2] = "Mine";
})(JobKind || (JobKind = {}));
export var JobState;
(function (JobState) {
    JobState[JobState["Seeking"] = 0] = "Seeking";
    JobState[JobState["Moving"] = 1] = "Moving";
    JobState[JobState["Working"] = 2] = "Working";
})(JobState || (JobState = {}));
...
export function makeJobSystem(hooks) {
    return function system_jobs(sim) {
        // Skip if no designations and no active jobs.
        if (sim.tick % ASSIGN_INTERVAL !== 0)
            return;
...
function assignNearestDesignation(sim, eid, hooks) {
    if (sim.designations.size === 0)
        return;
    const fromX = TilePos.tx[eid];
    const fromY = TilePos.ty[eid];
    let best = null;
    for (const [key, d] of sim.designations) {
        if (isAlreadyTargeted(sim, key, eid))
            continue;
        const dx = d.tx - fromX;
        const dy = d.ty - fromY;
...
function progressJob(sim, eid) {
    ...
    sim.events.push(`${designation.kind === 'chop' ? 'Chopped' : 'Mined'} at (${designation.tx},${designation.ty}).`);
    clearJob(eid);
}
function clearJob(eid) {
    Job.kind[eid] = JobKind.None;
    Job.state[eid] = JobState.Seeking;
    Job.progress[eid] = 0;
    Job.targetEid[eid] = 0;
}
function isAlreadyTargeted(sim, key, ignoreEid) {
    const eids = wardenQuery(sim.ecs);
    for (let i = 0; i < eids.length; i++) {
        const eid = eids[i];
        if (eid === ignoreEid)
        ...
        if (!hasComponent(sim.ecs, Job, eid))
            continue;
        if (Job.kind[eid] === JobKind.None)
            continue;
        if (Job.targetEid[eid] === key)
            return true;
    }
    return false;
}
